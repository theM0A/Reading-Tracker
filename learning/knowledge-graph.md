...
| Getters and setters | practicing | 2026-09-07 | 2026-09-07 | Task 3.4: wrote `getTotalPages()` correctly on the first attempt after predicting the signature shape unprompted. `setCurrentPage(int newPage)` initially left with an empty body — filled in correctly (`this.currentPage = newPage;`) once asked what line was missing. Self-added `getTitle()` and `getCurrentPage()` unprompted once `Main.java` needed them, applying the pattern independently. Post-task quiz (prepping task 3.5's status getter/setter) needed three rounds of correction: wrote `String` for a field that's actually the `BookStatus` enum, then wrote the bare keyword `enum` as a type (category, not a real type name), before landing `public BookStatus getStatus()` / `public void setStatus(BookStatus status)`. Real code written and applied, but not yet a clean quiz pass — stays at practicing. |
