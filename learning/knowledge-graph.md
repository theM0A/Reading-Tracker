...
| Getters and setters | practicing | 2026-09-07 | 2026-09-08 | Task 3.4: wrote `getTotalPages()` correctly on the first attempt after predicting the signature shape unprompted. `setCurrentPage(int newPage)` initially left with an empty body — filled in correctly (`this.currentPage = newPage;`) once asked what line was missing. Self-added `getTitle()` and `getCurrentPage()` unprompted once `Main.java` needed them, applying the pattern independently. Post-task quiz (prepping task 3.5's status getter/setter) needed three rounds of correction: wrote `String` for a field that's actually the `BookStatus` enum, then wrote the bare keyword `enum` as a type (category, not a real type name), before landing `public BookStatus getStatus()` / `public void setStatus(BookStatus status)`. Task 3.5: wrote both into `Book.java` for real, correctly on the first attempt, matching the file's existing pattern. Asked a good design question unprompted afterward — why `setStatus` used the same parameter name as the field (requiring `this.`) while `setCurrentPage` didn't — and correctly concluded, once shown the alternative, that both styles are valid and that matching the field name plus `this.` is the more common professional convention. Real code and real design reasoning, but the parameter-shadowing explanation itself still needed one correction on the post-task quiz — stays at practicing. |
...
**Most recent `understood` upgrade (2026-09-09, task 3.6):** Conditionals — moved
